...
class Predictor:
    ...
    def predict(self):
        ...
        self.populate(ensemble, data_set_rules)
        ...
        for iter_index in range(1, self.iterations+1):
            ...
            for myi, seq in enumerate(self.pop):
                ...
                for model in ensemble:
                    _, tempF = objF.eval(seq.pattern, 0, model, True)
                    fitness += tempF
    ...
    def populate(self, ensemble, dataset):
        ...
        for i in range(self.popSize):
            ...
            known_pattern = R.choice(dataset)
            pattern = copy.deepcopy(known_pattern)
            while pattern in dataset:
                p = list(pattern)
                R.shuffle(p)
                pattern = ''.join(p)
